...
private:
  ...
  static constexpr const char *INITIAL_XFEN =
  ...
  std::string calculateNewXfen(const std::string &currentXfen, int fromX,
  ...
  std::string boardToXfen(const char board[10][9], const std::string &turn,
                          int moveCount);

public:
  // Validate XFEN format for custom board setup (public for use in handlers)
  bool isValidXfen(const std::string &xfen);

  // Convert custom board setup (JSON map) to XFEN string (public for use in
  // handlers)
  std::string customBoardSetupToXfen(const nlohmann::json &customBoardSetup,
  ...
  GameResult createGameWithColors(const std::string &redPlayer,
                                  ...
                                  const std::string &timeControl, bool rated);
  ...
  // timeLimit: Time limit in seconds (0 for unlimited, overrides timeControl default)
  ...
  // Thực hiện nước đi
  GameResult makeMove(const std::string &username, const std::string &gameId,
                      int fromX, int fromY, int toX, int toY,
                      const std::string &piece = "",
                      const std::string &captured = "",
                      const std::string &notation = "",
                      const std::string &xfenAfter = "", int timeTaken = 0);

  // Kết thúc game với result cụ thể (dùng cho draw agreement từ protocol,
  // checkmate, etc.) result: "red_win", "black_win", "draw", "abandoned"
  // termination: "checkmate", "resignation", "timeout", "draw_agreement", etc.
  GameResult endGame(const std::string &gameId, const std::string &result,
  ...
  GameResult respondToDraw(const std::string &username,
                           const std::string &gameId, bool accept);

  // ============ Rematch Operations ============

  // Yêu cầu rematch sau khi game kết thúc
  GameResult requestRematch(const std::string &username,
                            const std::string &gameId);

  // Phản hồi yêu cầu rematch
  // accept: true = chấp nhận rematch (tạo game mới), false = từ chối
  GameResult respondToRematch(const std::string &username,
                              const std::string &gameId, bool accept);
  ...
  GameResult listGames(const std::string &username,
  ...
  GameResult getGameHistory(const std::string &username, int limit = 50,
                            int offset = 0);
