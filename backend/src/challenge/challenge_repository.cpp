...
string ChallengeRepository::create(const Challenge &challenge) {
  try {
    ...
    auto doc =
        document{} << "challenger_username" << challenge.challenger_username
                   << "challenged_username" << challenge.challenged_username
                   << "time_control" << challenge.time_control << "rated"
                   << challenge.rated << "status" << challenge.status
                   << "message" << challenge.message << "created_at"
                   << bsoncxx::types::b_date{challenge.created_at}
                   << "expires_at"
                   << bsoncxx::types::b_date{challenge.expires_at} << finalize;
...
optional<Challenge> ChallengeRepository::findById(const string &challengeId) {
  try {
    ...
    challenge.challenger_username =
        string(view["challenger_username"].get_string().value);
    challenge.challenged_username =
        string(view["challenged_username"].get_string().value);
    ...
    if (view["responded_at"] &&
        view["responded_at"].type() == bsoncxx::type::k_date) {
...
optional<Challenge>
ChallengeRepository::findPendingBetweenUsers(const string &challenger,
  ...
  try {
    ...
    auto result =
        challenges.find_one(document{} << "challenger_username" << challenger
...
bool ChallengeRepository::updateStatus(const string &challengeId,
  ...
  try {
    ...
    setDoc.append(bsoncxx::builder::basic::kvp("responded_at",
                                               bsoncxx::types::b_date{now}));

    if (gameId.has_value()) {
      setDoc.append(bsoncxx::builder::basic::kvp("game_id",
                                                 bsoncxx::oid(gameId.value())));
    }
...
vector<Challenge> ChallengeRepository::findByUser(const string &username,
  ...
  try {
    ...
    if (filter == "sent") {
      ...
      queryBuilder << "$or" << open_array << open_document
                   << "challenger_username" << username << close_document
                   << open_document << "challenged_username" << username
                   << close_document << close_array << "status" << "pending";
    } else { // "all"
      queryBuilder << "$or" << open_array << open_document
                   << "challenger_username" << username << close_document
                   << open_document << "challenged_username" << username
    ...
    for (auto &&doc : cursor) {
      ...
      challenge.challenger_username =
          string(doc["challenger_username"].get_string().value);
      challenge.challenged_username =
          string(doc["challenged_username"].get_string().value);
...
int ChallengeRepository::deleteExpired() {
  try {
    ...
    auto result = challenges.delete_many(document{}
                                         << "status" << "pending"
