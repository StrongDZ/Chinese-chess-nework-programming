...
// Global controllers
static AuthController *g_auth_controller = nullptr;
static FriendController *g_friend_controller = nullptr;
static GameController *g_game_controller = nullptr;
static PlayerStatController *g_player_stat_controller = nullptr;
      ...
      case MessageType::USER_STATS: {
        ...
        break;
          ...
          case MessageType::DRAW_REQUEST: {
            ...
            if (g_clients.count(opp)) {
              if (g_clients.count(opp)) {
                sendMessage(opp, MessageType::DRAW_REQUEST,
                            DrawRequestPayload{});
                sendMessage(
                    fd, MessageType::INFO,
            ...
            if (g_clients.count(opp)) {
              if (g_clients.count(opp)) {
                ...
                if (p.accept_draw) {
                  ...
                  sendMessage(opp, MessageType::GAME_END, gp);
                  g_clients[fd].in_game = false;
                  g_clients[fd].opponent_fd = -1;
                  g_clients[opp].in_game = false;
                  g_clients[opp].opponent_fd = -1;
                  g_clients[fd].in_game = false;
                  g_clients[fd].opponent_fd = -1;
          ...
          case MessageType::REMATCH_REQUEST: {
            ...
            if (g_clients.count(opp)) {
              if (g_clients.count(opp)) {
                sendMessage(opp, MessageType::REMATCH_REQUEST,
            ...
            if (g_clients.count(opp)) {
              if (g_clients.count(opp)) {
                ...
                if (p.accept_rematch) {
                  ...
                  gs1.opponent = g_clients[opp].username;
                  gs1.opponent = g_clients[opp].username;
                  ...
                  // Swap colors
                  g_clients[opp].is_red = !g_clients[opp].is_red;
                  g_clients[opp].is_red = !g_clients[opp].is_red;
          ...
          default:
            ...
            case MessageType::LOGOUT: {
              auto &sender = g_clients[fd];
              if (!sender.username.empty() &&
                  g_username_to_fd.count(sender.username)) {
            ...
            case MessageType::CHALLENGE_CANCEL: {
              ...
              if (it != g_username_to_fd.end()) {
                ...
                if (it != g_username_to_fd.end()) {
                  ...
                  sendMessage(it->second, MessageType::CHALLENGE_CANCEL,
                              forwardPayload);
                }
                sendMessage(
                    fd, MessageType::INFO,
              ...
              if (it == g_username_to_fd.end()) {
                ...
                if (!g_clients.count(target_fd)) {
                  ...
                  if (target_player.in_game && target_player.opponent_fd >= 0 &&
                      g_clients.count(target_player.opponent_fd)) {
                    info["opponent"] =
                        g_clients[target_player.opponent_fd].username;
        g_clients.count(target_player.opponent_fd)) {
          info["opponent"] = g_clients[target_player.opponent_fd].username;
        }
        else {
        ...
        break;
                ...
                default:
                  ...
                  void processAIMatch(const ParsedMessage &pm, int fd) {
                    void processAIMatch(const ParsedMessage &pm, int fd) {
                      // This function runs in thread pool - AI processing can
                      // take time
                      lock_guard<mutex> lock(g_clients_mutex);
                      auto &sender = g_clients[fd];
                      lock_guard<mutex> lock(g_clients_mutex);
                        ...
                        if (!pm.payload.has_value() ||
                          ...
                          try {
                            ...
                            // Notify client that AI is processing
                            {
                              ...
                              // This is where AI calculation happens (can take
                              // seconds) Example: AIMove aiMove =
                              // aiModule.calculateMove(gamemode, boardState);
                              ...
                              // For now, send placeholder response
                              {
                                ...
                                if (g_clients.count(fd)) {
                                  ...
                                  if (g_clients.count(fd)) {
                                    ...
                                    sendMessage(
                                        fd, MessageType::ERROR,
                                        ErrorPayload{"AI_MATCH: AI module not "
                                                     "integrated yet"});
                                  }
                                }
                              }
                              ...
                              // ===================== //
                              void handleLogin(const ParsedMessage &pm,
                                               int fd) {
                                lock_guard<mutex> lock(g_clients_mutex);
                                auto &sender = g_clients[fd];
                                void handleLogin(const ParsedMessage &pm,
                                  ...
                                  auto &sender = g_clients[fd];
                                  if (!pm.payload.has_value() ||
                                      !holds_alternative<LoginPayload>(
                                          *pm.payload)) {
                                    sendMessage(
                                        fd, MessageType::ERROR,
                                        ErrorPayload{"LOGIN requires username "
                                                     "and password"});
                                    return;
                                  }
                                  ...
                                  try {
                                    const auto &p =
                                        get<LoginPayload>(*pm.payload);

                                    // Convert payload to nlohmann::json for
                                    // controller
                                    ...
                                    nlohmann::json response =
                                        g_auth_controller->handleLogin(request);
                                ...
                                void handleChallenge(const ParsedMessage &pm,
                                                     int fd) {
                                  lock_guard<mutex> lock(g_clients_mutex);
                                  auto &sender = g_clients[fd];
                                  void handleChallenge(const ParsedMessage &pm,
                                    ...
                                    if (sender.username.empty()) {
                                      sendMessage(
                                          fd, MessageType::ERROR,
                                          ErrorPayload{"Please LOGIN before "
                                                       "challenging"});
                                      return;
                                    }
                                    if (!pm.payload.has_value() ||
                                      ...
                                      sendMessage(
                                          fd, MessageType::ERROR,
                                          ErrorPayload{"CHALLENGE_REQUEST "
                                                       "requires username"});
                                      return;
                                    }
                                    try {
                                      const auto &p =
                                          get<ChallengeRequestPayload>(
                                              *pm.payload);
                                      const string &target =
                                          p.to_user; // Client -> Server uses
                                                     // to_user
                                      if (target.empty()) {
                                        sendMessage(
                                            fd, MessageType::ERROR,
                                      ...
                                      if (it == g_username_to_fd.end()) {
                                        ...
                                        // Forward challenge to target with
                                        // from_user field
                                        ChallengeRequestPayload forwardPayload;
                                        forwardPayload.from_user =
                                            sender.username;
                                        forwardPayload.to_user = "";
                                        sendMessage(
                                            target_fd,
                                      ...
                                      void handleChallengeResponse(
                                        ...
                                        if (!pm.payload.has_value() ||
                                          ...
                                          sendMessage(
                                              fd, MessageType::ERROR,
                                              ErrorPayload{
                                                  "CHALLENGE_RESPONSE requires "
                                                  "username and accept"});
                                          return;
                                        }
                                        try {
                                          const auto &p =
                                              get<ChallengeResponsePayload>(
                                                  *pm.payload);
                                          const string &challengerName =
                                              p.to_user; // Client -> Server
                                                         // uses to_user
                                          ...
                                          if (!p.accept) {
                                            // Decline challenge - forward to
                                            // challenger
                                            auto it = g_username_to_fd.find(
                                                challengerName);
                                            if (it != g_username_to_fd.end()) {
                                              auto it = g_username_to_fd.find(
                                                  challengerName);
                                              if (it !=
                                                  g_username_to_fd.end()) {
                                                ChallengeResponsePayload
                                                    forwardPayload;
                                                forwardPayload.from_user =
                                                    sender.username;
                                            ...
                                            if (it == g_username_to_fd.end()) {
                                              ...
                                              if (!g_clients.count(
                                                ...
                                                // Initialize game state for
                                                // both players (PvP game)
                                                // Challenger (người thách đấu)
                                                // plays Red, Accepter (người
                                                // chấp nhận) plays Black Use
                                                // EASY as dummy difficulty for
                                                // PvP (not used for AI)
                                                g_game_state.initializeGame(
                                                    challenger_fd, fd,
                                                    AIDifficulty::EASY);
                                                g_game_state.initializeGame(
                                                    fd, challenger_fd,
                                                ...
                                                // Set player colors: Challenger
                                                // is Red (goes first), Accepter
                                                // is Black
                                                ...
                                                gs1.game_mode =
                                                    "classic"; // Default, can
                                                               // be enhanced
                                                gs1.opponent_data = nlohmann::
                                                    json(); // Can be enhanced
                                                            // with player stats
                                                gs2.opponent = sender.username;
                                                gs2.game_mode = "classic";
                                                gs2.opponent_data = nlohmann::
                                                    json(); // Can be enhanced
                                                            // with player stats
                                            ...
                                            void handleMove(
                                              ...
                                              void handleMove(
                                                ...
                                                // Check if AI game (opponent_fd
                                                // == -1) or PvP game
                                                bool is_ai_game =
                                                    (sender.opponent_fd ==
                                                     -1) &&
                                                    g_game_state.hasGame(fd);

                                                if (!is_ai_game) {
                                                  // PvP game - check opponent
                                                  // exists
                                                  ...
                                                  // (works for both AI and PvP
                                                  // games)
                                                  char board[10][9];
                                                  bool has_board =
                                                      g_game_state
                                                          .getCurrentBoardArray(
                                                              fd, board);
                                                  ...
                                                  // 2. Check if it's player's
                                                  // turn (validate piece color
                                                  // matches player)
                                                  ...
                                                  // should move based on turn
                                                  // In AI game: player is
                                                  // always Red (goes first),
                                                  // player_turn indicates
                                                  // player's turn In PvP game:
                                                  // use move count to determine
                                                  // turn (even = Red, odd =
                                                  // Black)
                                                  bool is_red_turn;
                                                  bool player_is_red =
                                                  ...
                                                  if (is_ai_game) {
                                                    // AI game: player is Red,
                                                    // player_turn indicates if
                                                    // it's player's turn
                                                    is_red_turn =
                                                        board_state.player_turn;
                                                  } else {
                                                    // PvP game: use total move
                                                    // count to determine turn
                                                    // Even number of moves (0,
                                                    // 2, 4...) = Red's turn
                                                    // (challenger) Odd number
                                                    // of moves (1, 3, 5...) =
                                                    // Black's turn (accepter)
                                                    int move_count =
                                                        board_state.moves
                                                            .size();
                                                    is_red_turn =
                                                  ...
                                                  // Check if it's this player's
                                                  // turn
                                                  bool should_be_red_turn =
                                                      is_red_turn;

                                                  // Validate piece color
                                                  // matches player and turn
                                                  ...
                                                  // 3. Update game state (for
                                                  // both AI and PvP)
                                                  if (!g_game_state.applyMove(
                                                          fd, move)) {
                                                    sendMessage(
                                                        fd, MessageType::ERROR,
                                                        ErrorPayload{
                                                  ...
                                                  // 5. If PvP, also update
                                                  // opponent's game state to
                                                  // keep them in sync
                                                  if (!is_ai_game) {
                                                    int opp =
                                                        sender.opponent_fd;
                                                    if (g_game_state.hasGame(
                                                            opp)) {
                                                      g_game_state.applyMove(
                                                          opp, move);
                                                    }
                                                  }
                                                            ...
                                                            try {
                                                              ...
                                                              handleAIMatch(
                                                                ...
                                                                handleAIMatch(
                                                                  ...
                                                                  gs.opponent_data =
                                                                      nlohmann::
                                                                          json(); // Can be enhanced with AI info
                                                                    ...
                                                                    if (!g_game_state
                                                                      ...
                                                                      sendMessage(
                                                                          ...
                                                                          MessageType::
                                                                              ERROR,
                                                                          ErrorPayload{
                                                                              "No game state found. This feature works with AI games."});
                                                                      return;
                                                                    }
                                                                    ...
                                                                    if (current_fen
                                                                      ...
                                                                      sendMessage(
                                                                          ...
                                                                          MessageType::
                                                                              ERROR,
                                                                          ErrorPayload{
                                                                              "Failed to get current game position"});
                                                                      return;
                                                                    }
                                                                    ...
                                                                    if (ucci_move
                                                                      ...
                                                                      sendMessage(
                                                                          ...
                                                                          MessageType::
                                                                              ERROR,
                                                                          ErrorPayload{
                                                                              "Failed to get move suggestion from AI"});
                                                                      return;
                                                                    }
                                                                                ...
                                                                                try {
                                                                                  ...
                                                                                  if (ai_move.from
                                                                                    ...
                                                                                    pushAIMessage(
                                                                                        ...
                                                                                        MessageType::
                                                                                            ERROR,
                                                                                        ErrorPayload{
                                                                                            "AI generated invalid move format"});
                                                                                    pushAIMessage(
                                                                                        player_fd,
                                                                                        MessageType::
                                                                                            ERROR,
                                                                                  ...
                                                                                  if (!g_game_state
                                                                                           .hasGame(
                                                                                               player_fd)) {
                                                                                    std::cout
                                                                                        << "[AI] Game ended while AI thinking, cancelling move"
                                                                                        << std::
                                                                                               endl;
                                                                                  ...
                                                                                  if (state_hash_after ==
                                                                                    ...
                                                                                    std::cerr
                                                                                        << "[AI] Warning: Game state changed while AI thinking (hash mismatch). "
                                                                                        << "Rejecting move to prevent race condition."
                                                                                        << std::
                                                                                               endl;
                                                                                    std::lock_guard<
                                                                                        std::
                                                                                        ...
                                                                                        .push(
                                                                                            {player_fd,
                                                                                             MessageType::
                                                                                                 ERROR,
                                                                                             ErrorPayload{
                                                                                                 "Game state changed, AI move rejected (please try again)"}});
                                                                                    g_ai_queue_cv
                                                                                        .notify_one();
                                                                                    return;
                                                                                  }
                                                                                  ...
                                                                                  if (has_board) {
                                                                                    ...
                                                                                    if (!GameStateManager::
                                                                                            quickValidateMove(
                                                                                                ...
                                                                                                board)) {
                                                                                      std::cerr
                                                                                          << "[AI] CRITICAL ERROR: AI generated invalid move format: "
                                                                                          << ucci_move
                                                                                          << " - Engine panic detected!"
                                                                                          << std::
                                                                                                 endl;
                                                                                      std::lock_guard<
                                                                                          std::
                                                                                          ...
                                                                                          .push(
                                                                                              {player_fd,
                                                                                               MessageType::
                                                                                                   ERROR,
                                                                                               ErrorPayload{
                                                                                                   "AI engine error: invalid move format (critical error)"}});
                                                                                      g_ai_queue_cv
                                                                                          .notify_one();
                                                                                      return;
                                                                                    }
                                                                                    ...
                                                                                    std::cout
                                                                                        << "[AI] Note: Could not get board, but state hash matches. "
                                                                                        << "Trusting Pikafish engine validation."
                                                                                        << std::
                                                                                               endl;
                                                                                  }
