...
FriendRepository::mapDocToRelation(const bsoncxx::document::view &doc) {
  try {
    FriendRelation rel;
    ...
    if (doc["accepted_at"] &&
        doc["accepted_at"].type() == bsoncxx::type::k_date) {
      rel.accepted_at = chrono::system_clock::time_point(
          chrono::milliseconds(doc["accepted_at"].get_date().value.count()));
    }
    if (doc["blocked_at"] &&
        doc["blocked_at"].type() == bsoncxx::type::k_date) {
      rel.blocked_at = chrono::system_clock::time_point(
          chrono::milliseconds(doc["blocked_at"].get_date().value.count()));
    }
    if (doc["games_played_together"] &&
        doc["games_played_together"].type() == bsoncxx::type::k_int32) {
      rel.games_played_together =
          doc["games_played_together"].get_int32().value;
    }
    return rel;
...
optional<FriendRelation>
FriendRepository::findRelation(const string &user, const string &friendName) {
  try {
    auto db = mongoClient.getDatabase();
    auto friends = db["friends"];
    auto res =
        friends.find_one(document{} << "user_name" << user << "friend_name"
                                    << friendName << finalize);
    if (!res)
      return nullopt;
...
string FriendRepository::createRelation(const FriendRelation &relation) {
  try {
    ...
    doc << "user_name" << relation.user_name << "friend_name"
        << relation.friend_name << "status" << relation.status << "created_at"
        << bsoncxx::types::b_date{relation.created_at}
        << "games_played_together" << relation.games_played_together;
    if (relation.accepted_at) {
      doc << "accepted_at"
          << bsoncxx::types::b_date{relation.accepted_at.value()};
    }
    if (relation.blocked_at) {
      doc << "blocked_at"
          << bsoncxx::types::b_date{relation.blocked_at.value()};
    }
...
bool FriendRepository::updateStatus(const string &user,
  ...
  try {
    ...
    if (setAcceptedTime) {
      setDoc.append(bsoncxx::builder::basic::kvp("accepted_at",
                                                 bsoncxx::types::b_date{now}));
    }
    if (setBlockedTime) {
      setDoc.append(bsoncxx::builder::basic::kvp("blocked_at",
                                                 bsoncxx::types::b_date{now}));
    }
    ...
    auto res =
        friends.update_one(document{} << "user_name" << user << "friend_name"
                                      << friendName << finalize,
                           updateDoc.view());
...
bool FriendRepository::deleteRelation(const string &user,
                                      const string &friendName,
                                      const optional<string> &statusFilter) {
  try {
    auto db = mongoClient.getDatabase();
    auto friends = db["friends"];
...
vector<FriendRelation> FriendRepository::findAccepted(const string &username) {
  ...
  try {
    auto db = mongoClient.getDatabase();
    auto friends = db["friends"];
    auto cursor = friends.find(document{} << "user_name" << username << "status"
                                          << "accepted" << finalize);
    for (auto &&doc : cursor) {
...
FriendRepository::findPendingReceived(const string &username) {
  vector<FriendRelation> out;
  try {
    auto db = mongoClient.getDatabase();
    auto friends = db["friends"];
    auto cursor = friends.find(document{} << "friend_name" << username
...
vector<FriendRelation>
FriendRepository::findPendingSent(const string &username) {
  vector<FriendRelation> out;
  try {
    auto db = mongoClient.getDatabase();
    auto friends = db["friends"];
...
vector<FriendRelation>
FriendRepository::searchFriends(const string &username,
  ...
  try {
    ...
    auto friends = db["friends"];
    auto cursor = friends.find(
        document{} << "user_name" << username << "status" << "accepted"
                   << "friend_name" << open_document << "$regex" << searchQuery
