...
string GameService::boardToXfen(const char board[10][9], const string& turn, int moveCount) {
    stringstream ss;
...
bool GameService::isValidXfen(const string& xfen) {
    ...
    if (!turnPart.empty() && turnPart != "w" && turnPart != "b" && 
        turnPart != "r" && turnPart != "W" && turnPart != "B" && turnPart != "R") {
        return false;
    }
    ...
    while (getline(rowStream, row, '/')) {
        ...
        for (char c : row) {
            if (c >= '1' && c <= '9') {
                ...
                // Valid piece characters: r,n,b,a,k,c,p (lowercase=black, uppercase=red)
                char lower = tolower(c);
                if (lower != 'r' && lower != 'n' && lower != 'b' && 
                    lower != 'a' && lower != 'k' && lower != 'c' && lower != 'p') {
                    return false;
                }
                colCount++;
            } else {
        ...
        if (colCount != 9) return false; // Each row must have exactly 9 columns
        rowIndex++;
    }
...
string GameService::calculateNewXfen(const string& currentXfen,
                                      int fromX, int fromY,
                                      int toX, int toY,
...
GameResult GameService::makeMove(const string& username,
    ...
    if (calculatedXfen.empty()) {
        calculatedXfen = calculateNewXfen(game.xfen, fromX, fromY, toX, toY, nextTurn);
    }
    ...
    bool updated = repository.updateAfterMove(gameId, move, nextTurn, redTime, blackTime, calculatedXfen);
...
GameResult GameService::respondToDraw(const string& username, const string& gameId, bool accept) {
    GameResult result;
    result.success = false;
...
GameResult GameService::requestRematch(const string& username, const string& gameId) {
    GameResult result;
    result.success = false;
    ...
    if (username != archivedGame.red_player && username != archivedGame.black_player) {
        result.message = "You are not a player in this game";
        return result;
    }
    ...
    if (!archivedGame.rematch_offered_by.empty()) {
        if (archivedGame.rematch_offered_by == username) {
            ...
            result.message = "There is already a pending rematch request from opponent";
            return result;
        }
    }
...
GameResult GameService::respondToRematch(const string& username, const string& gameId, bool accept) {
    GameResult result;
    result.success = false;
    ...
    if (accept) {
        ...
        auto createResult = createGameWithColors(newRedPlayer, newBlackPlayer, 
...
GameResult GameService::getGameHistory(const string& username, int limit, int offset) {
    GameResult result;
    result.success = false;
