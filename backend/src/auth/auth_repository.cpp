...
AuthRepository::AuthRepository(MongoDBClient &mongo) : mongoClient(mongo) {}
...
string AuthRepository::createUser(const string &username,
                                  const string &passwordHash, int avatarId) {
  auto db = mongoClient.getDatabase();
  auto users = db["users"];
  auto now = chrono::system_clock::now();

  auto userDoc = document{} << "username" << username << "password_hash"
                            << passwordHash << "avatar_id" << avatarId
                            << "status" << "active"
                            << "is_online" << false << "created_at"
                            << bsoncxx::types::b_date{now} << "last_login"
                            << bsoncxx::types::b_date{now} << finalize;
...
bool AuthRepository::updateAvatar(const string &username, int avatarId) {
  ...
  auto result =
      users.update_one(document{} << "username" << username << finalize,
                       document{} << "$set" << open_document << "avatar_id"
                                  << avatarId << close_document << finalize);
...
bool AuthRepository::updateOnlineStatus(const string &username, bool isOnline) {
  ...
  auto result =
      users.update_one(document{} << "username" << username << finalize,
                       document{} << "$set" << open_document << "is_online"
                                  << isOnline << close_document << finalize);
...
bool AuthRepository::updateLastLogin(const string &username) {
  ...
  auto result =
      users.update_one(document{} << "username" << username << finalize,
                       document{} << "$set" << open_document << "last_login"
                                  << bsoncxx::types::b_date{now} << "is_online"
...
void AuthRepository::createDefaultStats(const string &username) {
  ...
  for (const auto &timeControl : timeControls) {
    auto statDoc = document{}
                   ...
                   << "rating" << initialRating << "highest_rating"
                   << initialRating << "lowest_rating" << initialRating << "rd"
                   << 350.0 << "volatility" << 0.06 << "total_games" << 0
                   << "wins" << 0 << "losses" << 0 << "draws" << 0
                   << "win_streak" << 0 << "longest_win_streak" << 0
                   << "total_playtime" << 0 << "last_game_time"
                   << bsoncxx::types::b_date{now} << finalize;
