...
class AI:
    ...
    def predict_move(self, fen_position: str, difficulty: AIDifficulty) -> Optional[Move]:
        ...
        with self.engine_lock:
            ...
            # Read best move with timeout (time_ms + 2s buffer)
            ...
            if "bestmove" in response:
                ...
                try:
                    ...
                    if bestmove_idx + 1 < len(parts):
                        ...
                        if move:
                            print(f"[AI] Parsed move: from=({move.from_pos.row},{move.from_pos.col}) to=({move.to_pos.row},{move.to_pos.col})", file=sys.stderr)
                        return move
